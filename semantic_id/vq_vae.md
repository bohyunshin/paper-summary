...
$$
z_q(x) = e_k, \quad \text{where} \quad k = \arg\min_j \lVert z_e(x) - e_j \rVert_2
\tag{2}
$$
...
$$
\mathcal{L} = \text{재구성 오차} + \mathrm{KL}\big(q(z\mid x) \,\Vert\, p(z)\big)
$$
...
진짜 사후분포 $p(z\mid x)$는 계산 불가능 (분모 $p(x) = \int p(x\mid z)p(z)\,dz$가 적분 불가능). 그래서 $q(z\mid x)$라는 **근사 분포**를 인코더로 학습시켜 $p(z\mid x)$를 대신함 — 이것이 "variational"의 의미.

### KL term이 두 분포를 묶는 지점

$$
\mathrm{KL}\big(q(z\mid x) \,\Vert\, p(z)\big)
$$
...
$$
\mathrm{KL}\big(q(z\mid x) \,\Vert\, p(z\mid x)\big)
$$
...
$$
\log p(x) = \mathrm{ELBO} + \mathrm{KL}\big(q(z\mid x) \,\Vert\, p(z\mid x)\big)
$$
...
$\log p(x)$를 최대화하고 싶지만 $p(x) = \int p(x\mid z)p(z)\,dz$는 적분 불가능.

### 1단계: $q(z\mid x)$를 끼워넣기

$$
\log p(x) = \log \int p(x,z)\,dz = \log \int q(z\mid x) \cdot \frac{p(x,z)}{q(z\mid x)}\,dz = \log \mathbb{E}_{q(z\mid x)}\!\left[\frac{p(x,z)}{q(z\mid x)}\right]
$$
...
&= \underbrace{\mathbb{E}_{q(z\mid x)}\big[\log p(x\mid z)\big]}_{\text{재구성 항}} \;-\; \underbrace{\mathrm{KL}\big(q(z\mid x)\,\Vert\,p(z)\big)}_{\text{KL term}}
\end{aligned}
$$
...
&= \log p(x) - \mathrm{KL}\big(q(z\mid x)\,\Vert\,p(z\mid x)\big)
\end{aligned}
$$
...
$$
\mathrm{ELBO} = \log p(x) - \mathrm{KL}\big(q(z\mid x)\,\Vert\,p(z\mid x)\big)
$$
...
**3) 계산 가능성이 정반대**: $\mathrm{KL}(q\Vert p(z\mid x))$는 $p(z\mid x)$가 들어있어 직접 계산 불가능. ELBO는 $p(x\mid z), p(z), q(z\mid x)$만으로 표현되어 계산 가능($p(z\mid x)$가 전혀 안 나타남). 계산 불가능한 KL을 직접 줄이는 문제를, 계산 가능한 ELBO를 올리는 문제로 치환.
...
$$
\mathrm{KL}\big(q(z\mid x)\,\Vert\,p(z)\big) = \sum_{k=1}^{K} q(z=k\mid x)\,\log\!\frac{q(z=k\mid x)}{p(z=k)}
$$

### 두 분포 대입

**$q(z\mid x)$** (식 1, one-hot, $k^*$를 선택된 인덱스라 하면):
...
$q(z=k\mid x)=0$인 항들은 모두 0이 되어 사라짐. $k=k^*$인 항 하나만 남음:

$$
\mathrm{KL} = q(z=k^*\mid x)\cdot\log\!\frac{q(z=k^*\mid x)}{p(z=k^*)} = 1 \cdot \log\!\frac{1}{1/K} = \log K
$$

### 왜 "constant"인가

$k^*$가 무엇이든(인코더가 어떤 입력 $x$에 대해 어떤 코드를 고르든) 결과는 항상 $\log K$. $q(z=k^*\mid x)$는 항상 1, $p(z=k^*)$는 균등분포라 $k^*$가 몇 번이든 항상 $1/K$.
...
| $\mathrm{KL}(q\Vert p)$ | $x$에 따라 값 달라짐, 그래디언트 있음 | 항상 $\log K$, 그래디언트 0 |
...
$$
\mathcal{L} = \underbrace{\log p(x\mid z_q(x))}_{\text{항 1: 재구성}} + \underbrace{\lVert \mathrm{sg}[z_e(x)] - e \rVert_2^2}_{\text{항 2: VQ loss}} + \underbrace{\beta\lVert z_e(x) - \mathrm{sg}[e] \rVert_2^2}_{\text{항 3: commitment loss}}
\tag{3}
$$
...
$$
\text{Loss} = -\lVert x - \hat{x} \rVert^2,\qquad \hat{x} = \mathrm{Decoder}_{\theta_{\text{dec}}}(z_q(x)),\qquad z_q(x) = e_{k^*}
$$
...
codebook 항목 $e_i$에 가장 가�게 할당된 인코더 출력들의 집합을 $\{z_{i,1}, \dots, z_{i,n_i}\}$라 하면, loss는:
...
$$
q(z=k\mid x) = \mathbb{1}[k=k^*], \qquad k^* = \arg\min_j \lVert z_e(x)-e_j \rVert
$$
...
$$
\mathrm{ELBO} = \mathbb{E}_{q(z\mid x)}\big[\log p(x\mid z)\big] - \mathrm{KL}\big(q(z\mid x)\,\Vert\,p(z)\big)
$$

**기댓값 항 풀기**: $q(z\mid x)$가 one-hot이므로 $K$개 항의 합이 단 하나의 항으로 붕괴:

$$
\mathbb{E}_{q(z\mid x)}\big[\log p(x\mid z)\big] = \sum_k q(z=k\mid x)\log p(x\mid z=k) = 1\cdot\log p(x\mid z=k^*) = \log p(x\mid z_q(x))
$$

여기서 $z_q(x) = e_{k^*}$. → **식 3의 항 1이 정확히 여기서 나옴** — q(z|x)가 one-hot이라는 가정 때문에 기댓값이 선택된 하나의 항으로 붕괴.

**KL 항**: [9번 섹션](#9-kl-divergence가-상수-log-k가-되는-이유-직접-계산)에서 유도한 대로:

$$
\mathrm{KL}\big(q(z\mid x)\,\Vert\,p(z)\big) = \log K \quad \text{(상수, 그래디언트 0이므로 제거)}
$$
...
$$
\mathcal{L} = \underbrace{\frac{1}{2\sigma^2}\lVert x - D(z_q(x)) \rVert^2}_{\text{ELBO에서 유도됨 (reconstruction)}} + \underbrace{\lVert \mathrm{sg}[z_e(x)] - e_{k^*} \rVert^2}_{\text{ELBO 밖에서 추가 (codebook 학습용)}} + \underbrace{\beta\lVert z_e(x) - \mathrm{sg}[e_{k^*}] \rVert^2}_{\text{ELBO 밖에서 추가 (commitment)}}
$$
...
$$
$$
k^* = \arg\min_j \lVert z_e(x) - e_j \rVert \quad \text{(forward에서 결정되는 정수, 미분 대상 아님)}
$$
$$
z_q(x) = e_{k^*}
$$
...
$$
\mathcal{L}_1 = \lVert x - \hat{x} \rVert^2 \qquad \mathcal{L}_2 = \lVert \mathrm{sg}[z_e(x)] - e_{k^*} \rVert^2 \qquad \mathcal{L}_3 = \beta\lVert z_e(x) - \mathrm{sg}[e_{k^*}] \rVert^2
$$
...
### 2) Codebook $e_{k^*}$에 대한 그래디언트

**$\mathcal{L}_1$에서**: straight-through estimator 정의상 $e_{k^*}$ 자체로는 그래디언트가 전달되지 않음.

$$
\frac{\partial \mathcal{L}_1}{\partial e_{k^*}} = 0
$$

**$\mathcal{L}_2$에서**: $\mathrm{sg}[z_e(x)]$는 상수 취급, $e_{k^*}$만 미분 대상.

$$
\mathcal{L}_2 = \lVert \mathrm{sg}[z_e(x)] - e_{k^*}\rVert^2 = \sum_d (z_e(x)_d - e_{k^*,d})^2
$$
$$
\frac{\partial \mathcal{L}_2}{\partial e_{k^*}} = 2(e_{k^*} - z_e(x))
$$

**$\mathcal{L}_3$에서**: $\mathrm{sg}[e_{k^*}]$가 상수이므로 $e_{k^*}$로 미분하면 0.

$$
\frac{\partial \mathcal{L}_3}{\partial e_{k^*}} = 0
$$

**다른 모든 $e_j\ (j\ne k^*)$**: 어느 항에도 등장하지 않으므로 그래디언트 0.

**합산**:

$$
\boxed{\frac{\partial \mathcal{L}}{\partial e_{k^*}} = 2(e_{k^*} - z_e(x)), \qquad \frac{\partial \mathcal{L}}{\partial e_j} = 0 \;(j\ne k^*)}
$$

그래디언트 디센트 업데이트:

$$
e_{k^*} \leftarrow e_{k^*} - \eta\cdot 2(e_{k^*}-z_e(x)) = e_{k^*} - 2\eta(e_{k^*}-z_e(x))
$$

$e_{k^*}-z_e(x)$만큼 빼주므로 $e_{k^*}$가 $z_e(x)$ 쪽으로 이동 — "e가 z_e(x) 쪽으로 끌려간다"는 직관이 수식으로 확인됨.
...
**$\mathcal{L}_3$에서**: $z_e(x)$만 미분 대상, $\mathrm{sg}[e_{k^*}]$는 상수.

$$
\mathcal{L}_3 = \beta\lVert z_e(x) - \mathrm{sg}[e_{k^*}] \rVert^2 = \beta\sum_d (z_e(x)_d - e_{k^*,d})^2
$$
$$
\frac{\partial \mathcal{L}_3}{\partial z_e(x)} = 2\beta(z_e(x) - e_{k^*})
$$

**합산 (체인룰 전 단계)**:

$$
\frac{\partial \mathcal{L}}{\partial z_e(x)} = \underbrace{-2(x-\hat{x})\cdot\frac{\partial \mathrm{Dec}}{\partial z_q(x)}}_{\mathcal{L}_1 \text{ (ST 경유)}} + \underbrace{2\beta(z_e(x)-e_{k^*})}_{\mathcal{L}_3 \text{ (commitment)}}
$$

**최종, $\theta_{\text{enc}}$까지 체인룰 완성**:

$$
\boxed{\frac{\partial \mathcal{L}}{\partial \theta_{\text{enc}}} = \left[-2(x-\hat{x})\cdot\frac{\partial \mathrm{Dec}}{\partial z_q(x)} + 2\beta(z_e(x)-e_{k^*})\right]\cdot\frac{\partial z_e(x)}{\partial \theta_{\text{enc}}}}
$$
...
| $e_{k^*}$ | 0 (ST가 끊음) | $2(e_{k^*}-z_e(x))$ | 0 (sg) | $\mathcal{L}_2$만 |
| $\theta_{\text{enc}}$ | ST로 우회 전달 | 0 (sg) | $2\beta(z_e(x)-e_{k^*})$ | $\mathcal{L}_1$(ST) + $\mathcal{L}_3$ |
...
$$
p(z) = \text{uniform (고정)}, \qquad \mathrm{KL}\big(q(z\mid x)\,\Vert\,p(z)\big) = \log K \text{ (상수, 그래디언트 0)}
$$
